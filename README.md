...
| Kendi Nyaga | Imported and cleaned the Formula 1 datasets in Power BI, standardized column names, corrected data types, replaced `\N` values with null values, removed unnecessary columns, performed column profiling, applied Merge Queries and created Reference Queries, performed **Group By** transformations with multiple aggregations, created summarized tables and a Date Table to support data modeling, prepared the dataset for analysis and visualization, and developed the final project presentation by designing the slide structure, summarizing dashboard insights, incorporating dashboard screenshots, and formatting the presentation for PDF submission. |
